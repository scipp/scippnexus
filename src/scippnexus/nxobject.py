...
class NXobject:
    ...
    def _assemble_leaf_properties(self, children: sc.DataGroup) -> sc.DataGroup:
        ...
        if detector_number is not None:
            detector_number = children[detector_number]
...
def is_off_geometry(dg: Any) -> bool:
    return isinstance(dg, sc.DataGroup) and all(
        key in dg for key in ('vertices', 'winding_order', 'faces'))
