...
@pytest.mark.externalfile()
def test_apply_to_transformations() -> None:
    filename = externalfile.get_path('2023/LOKI_60322-2022-03-02_2205_fixed.nxs')
    dg = transformations.load_transformations(filename)

    def gather_names(t: transformations.Transform) -> transformations.Transform:
        applied_to.append(t.name)
        return t
    ...
    applied_to = []
    transformations.apply_to_transformations(dg, gather_names)
    assert applied_to == paths
